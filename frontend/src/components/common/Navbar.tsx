...
const Navbar: React.FC = () => {
  ...
  const navLinks: NavLink[] = [
    ...
    { name: 'INITIATIVE', href: '#initiative', active: true },
    { name: 'BLOGS', href: '#blogs', active: false },
  ];

  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 px-3 sm:px-6 pt-3 font-sans selection:bg-yellow-400 selection:text-black transition-transform duration-300 ease-in-out ${isVisible ? 'translate-y-0' : '-translate-y-full'
        }`}
    >
        ...
        <nav className="hidden md:flex items-center gap-10 lg:gap-12 xl:gap-14">
          {navLinks.map((link: NavLink) => (
            ...
            >
              <a
                href={link.href}
                className={`font-bold uppercase transition-colors duration-200 py-1 text-[16px] lg:text-[17px] xl:text-[18px] tracking-wide ${link.active
                    ? 'text-yellow-400'
          ...
          <nav className="w-[96%] mx-auto bg-black/95 backdrop-blur-sm border border-neutral-800 rounded-3xl p-4 space-y-1.5 shadow-2xl">
            {navLinks.map((link: NavLink) => (
              <a
                ...
                className={`block py-3 px-4 text-sm font-bold tracking-wider rounded-xl transition-colors ${link.active
