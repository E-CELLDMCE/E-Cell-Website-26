...
const Home = () => {
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-red-600 selection:text-white">
      ...
      {/* Main Content Container with Crimson/Black backdrop behind navbar */}
      {/* Updated: pt-28 on mobile, pt-36 on desktop creates clear breathing space below the navbar */}
      <main
        className="pt-28 sm:pt-24 min-h-screen relative overflow-hidden"
        style={{
          backgroundColor: '#050000',
          backgroundImage: `
            radial-gradient(ellipse 95% 45% at 50% -5%, rgba(185, 15, 15, 0.65) 0%, rgba(110, 8, 8, 0.35) 45%, transparent 75%),
            radial-gradient(circle at 85% 15%, rgba(160, 10, 10, 0.3) 0%, transparent 50%),
      ...
      >
        ...
        {/* Updated: pt-6 creates proper spacing above the blue card */}
        <section id="home" className="relative z-10 w-full pt-6 pb-4 px-2 sm:px-4">
        ...
        <section
          id="about"
          className="relative w-full py-16 sm:py-20 px-4 sm:px-6 lg:px-8 text-center overflow-hidden"
          style={{
            backgroundImage: `
            ...
            <div className="relative z-10 max-w-6xl mx-auto">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 justify-items-center">
                {[1, 2, 3, 4].map((item) => (
                  <div
                    key={item}
                    className="w-36 h-36 rounded-[24px] p-[7px] flex items-center justify-center"
                    style={{
                      background:
                        "linear-gradient(145deg, #5c0505 0%, #b91c1c 45%, #450303 100%)",
                    }}
                  >
                    <div className="w-28 h-28 rounded-[15px] bg-white"></div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-10 md:gap-14 mt-16 items-end justify-items-center">
                {['Alumni', 'E-CELL', 'NEC', 'Enspire'].map((name) => (
                  <div key={name} className="flex flex-col items-center gap-3">
                    <div className="w-40 h-28 bg-neutral-900/40 rounded-lg flex items-center justify-center border border-neutral-800">
                      <span className="text-xs uppercase text-neutral-500 font-semibold">{name} Logo</span>
                    </div>
                  </div>
                ))}
              </div>
